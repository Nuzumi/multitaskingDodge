...
[CreateAssetMenu(menuName = "Events/NativeEvent")]
...
public abstract class NativeEvent<T1, T2, T3> : ScriptableObject
{

    private Action<T1, T2, T3> _event;


    public void Invoke(T1 data1, T2 data2, T3 data3)
    {
        if (_event != null)
            _event(data1, data2, data3);
    }
